import { PitStopParameters } from '../../types/types';
...
function init(): void {
  window.addEventListener('DOMContentLoaded', () => {
    ...
    // Choose file button
    const INPUT_FILE = document.getElementById("input-file")! as HTMLInputElement;
    ...
    // For keyboard users
    const LABEL_INPUT_FILE = document.querySelector('label[for="input-file"]')! as HTMLLabelElement;
    LABEL_INPUT_FILE.addEventListener("keydown", (event) => {
      if (event.key === " " || event.key === "Enter") {
        event.preventDefault(); // Prevent scrolling when pressing space
        INPUT_FILE.click(); // Trigger file selection
      }
    });
    ...
    document.getElementById("btn-upload")!.addEventListener('click', async () => {
      ...
      const CHECKED_CHECKBOXES = Array.from(CHECKBOXES).filter((checkbox) => (checkbox as HTMLInputElement).checked);
      const CHECKED_RADIO_BUTTONS = Array.from(RADIO_BUTTONS).filter((radio) => (radio as HTMLInputElement).checked);
    ...
    // Animations for closing the report
...
function returnPitStopParameters() {
  // Add the data-file-name attribute values to the parameters as an array
  ...
  const PARAMETERS: PitStopParameters = {
  ...
  CHECKBOXES_ACTION_LISTS.forEach((checkbox) => {
    if ((checkbox as HTMLInputElement).checked) {
      const ATTRIBUTE = (checkbox as HTMLInputElement).getAttribute('data-file-name');
      if (ATTRIBUTE) PARAMETERS.actionListsNames.push(ATTRIBUTE);
    }
  });
  CHECKBOXES_PREFLIGHT_PROFILES.forEach((radio) => {
    if ((radio as HTMLInputElement).checked) {
      const ATTRIBUTE = (radio as HTMLInputElement).getAttribute('data-file-name');
...
function createNotification(message: string, logLevel: 'info' | 'warning' | 'error' | 'success'): void {
  ...
  CONTAINER_NOTIFICATION.offsetWidth; // Trigger reflow
...
function returnReportHTML(REPORT_ITEMS: any[]): string {
  let HTML = '';
...
function pluralizeWord(count: number, singularWord: string): string {
...
// Format the PitStop report JSON data into a readable format
window.electron.onReport((report: any) => {
  console.log(report);
  const CONTAINER_REPORT_CONTENT = document.getElementById('container-report-content')!;
  ...
  // This attribute is used to save the file name of the report
...
window.electron.onNotification((message, logLevel: 'info' | 'warning' | 'error' | 'success') => {
